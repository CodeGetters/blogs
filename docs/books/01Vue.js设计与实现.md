...
代理：对一个对象基本语义的代理，它允许我们拦截并重新定义对一个对象的基本操作，类似读取、设置属性值的操作就属于基本语义操作
...
Proxy构造函数接收两个参数，第一个是被代理对象，第二个也是也是一个对象，一组夹子(trap)。其中 get 函数用来拦截读取操作，set 函数用来拦截设置操作

Reflect 是一个全局对象，其方法与 Proxy 的拦截器方法名字相同，但能接收第三个参数：指定接受者 receiver(可理解为函数调用过程中的 this)
```js
const obj = {
    obj : 1
}
...
// 2
// 读取到的值是 reveiver 对象的 foo 属性值
console.log(Reflect.get(obj,'foo',{foo:2}))
```

* JS对象及Proxy的工作原理

JS有常规对象(ordinary object)和异质对象(exotic object)两种对象

对象的实际语义是由对象的内部方法(internal method)指定的。所谓内部方法，指的是当我们对一个对象进行操作时在引擎内部低矮调用的方法，这些方法对于JS使用者来通过内部方法和内部槽来区分对象，例如函数对象会部署内部方法`[[Call]]`，而普通对象则不会

例如：访问对象属性
```js
obj.foo
```
引擎内部会调用`[[Get]]`这个内部方法来读取属性值(在ES中使用[[xxx]]来代表内部方法或内部槽)

如果一个对象需要作为函数调用，那么这个对象就逼i虚部署内部方法[[Call]]

内部方法具有多态性，类似面向对象中的多态。这就是说不同类型的对象可能部署了相同的内部方法，却有不同的逻辑。

常规对象必须要满足的三个条件：
1.对于内部方法，必须使用ESMA规范10.1.x节给出的定义实现
2.对于内部方法[[Call]]，必须使用ECMA规范10.2.1节给出的定义实现
3.对于内部方法[[Construst]]，必须使用ECMA规范10.2.2节给出的定义实现
如果不满足以上条件的就是异质对象---Proxy

怎么区分普通对象或函数：不同的对象部署相同的内部方法，但它们的行为可能不同，具体的不同体现在如果在创建代理对象时没有指定 get 拦截函数，那么当我们通过代理对象访问属性时，
代理对象的内部方法[[Get]]会调用原始对象的内部方法[[Get]]来获取属性值，这其实就是代理透明性质。


创建代理对象时指定的拦截函数，实际是用来自定义代理对象本身的内部方法和行为的，而不是用来指定被代理对象的内部方法和行为的。
